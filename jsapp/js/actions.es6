...
actions.charts = Reflux.createActions({
  setStyle: {
    children: [
      'completed',
      'failed',
...
actions.charts.setStyle.listen(function(assetId, details){
  dataInterface.patchAsset(assetId, {
    chart_styles: JSON.stringify(details),
  }).done(actions.charts.setStyle.completed)
    .fail(actions.charts.setStyle.failed);
});
